                    ...
                    <div class="flex flex-col gap-2 w-full max-w-2xl">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-2 gap-2">
                            ...
                            <x-input name="nip" value="{{ old('nip') }}" label="NIP:" size="w-full"
                                :required="true" />
                            <x-input name="nomor_sip" value="{{ old('nomor_sip') }}" label="Nomor SIP:" size="w-full"
                                :required="true" />
                            <x-input type="date" name="tanggal_sip" value="{{ old('tanggal_sip') }}" label="Tanggal SIP:"
                                size="w-full" :required="true" />
                            <x-input type="date" name="tmt" value="{{ old('tmt') }}" label="TMT:" size="w-full"
                                :required="true" />
                            <x-input name="nilai" value="{{ old('nilai') }}" label="Nilai:" size="w-full"
                                :required="true" />
                            <x-input.file name="file" label="Upload File SIP:" size="w-full md:col-span-2 lg:col-span-3"
                                :required="true" accept="application/pdf" />
                        </div>

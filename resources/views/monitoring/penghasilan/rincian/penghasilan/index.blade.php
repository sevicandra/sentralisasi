            ...
            <div class="row">
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <a href="{{ config('app.url') }}/monitoring/rincian/{{ $nip }}/penghasilan/{{ $item->tahun }}" class="btn btn-sm btn-outline-success @if (Request('thn') === null && $item->tahun === date('Y') || $item->tahun === request('thn'))active @endif ml-1 mt-1 mb-1">{{ $item->tahun }}</a>
                        @endforeach
                    </div>
                </div>
            </div>

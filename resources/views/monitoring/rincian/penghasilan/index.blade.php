        ...
        <div class="flex flex-col gap-2 flex-wrap py-2 px-4">
            <div class="w-full flex gap-1 flex-wrap">
                @foreach ($tahun as $item)
                    <a href="{{ config('app.url') }}/monitoring/rincian/{{ $nip }}/penghasilan/{{ $item->tahun }}"
                    ...
                    <x-table.body>
                        @foreach ($data as $item)
                            <tr class="*:border">
                                ...
                                <x-table.body.column class="text-right">{{ number_format($item->makan->netto, 0, ',', '.') }}</x-table.body.column>
                                <x-table.body.column class="text-right">{{ number_format($item->lembur->netto, 0, ',', '.') }}</x-table.body.column>
                                <x-table.body.column class="text-right">
                        ...
                        <tr class="*:border">
                            ...
                            <x-table.body.column class="text-right">{{ number_format($data->sum('makan.netto'), 0, ',', '.') }}</x-table.body.column>
                            <x-table.body.column class="text-right">{{ number_format($data->sum('lembur.netto'), 0, ',', '.') }}</x-table.body.column>
                            <x-table.body.column class="text-right">

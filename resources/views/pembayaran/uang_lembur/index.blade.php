            ...
            <div class="w-full flex gap-1 flex-wrap justify-between">
                <div class="flex gap-1 flex-wrap">
                    @foreach ($tahun as $item)
                        <a href="{{ config('app.url') }}/belanja-51/uang-lembur/index/{{ $item }}"
                            class="btn btn-xs btn-outline btn-primary @if ((!$thn && $item === date('Y')) || $item === $thn) btn-active @endif">{{ $item }}</a>
                    @endforeach
                </div>
                <div class="flex">
                    <a href="/belanja-51/uang-lembur/create" class="btn btn-xs btn-primary">Tambah</a>
                </div>
            </div>
        </div>
                            ...
                            <tr class="*:border">
                                ...
                                <x-table.body.column>
                                    <div class="flex gap-1 w-full h-full justify-center">
                                        <div>
                                            <form action="/belanja-51/uang-lembur/{{ $item->id }}/dokumen" method="post"
                                                target="_blank">
                                                @csrf
                                                @method('patch')
                                        ...
                                        <div>
                                            <form action="/belanja-51/uang-lembur/{{ $item->id }}/dokumen-excel"
                                ...
                                <x-table.body.column>
                                    ...
                                    @else
                                        <div class="flex gap-1 w-full h-full justify-center">
                                            <a href="/belanja-51/uang-lembur/{{ $item->id }}/edit" data-toggle="tooltip"
                                                data-placement="bottom" title="Ubah"
                                                class="btn btn-xs btn-primary">ubah</a>
                                            <form action="/belanja-51/uang-lembur/{{ $item->id }}/delete"
                                                method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                            ...
                                            <a href="/belanja-51/uang-lembur/{{ $item->id }}/kirim"

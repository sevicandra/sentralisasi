            ...
            <div class="w-full flex gap-1 flex-wrap justify-between">
                <div class="flex gap-1 flex-wrap">
                    @foreach ($tahun as $item)
                        <a href="{{ config('app.url') }}/belanja-51/wilayah/uang-lembur/{{ $item }}"
            ...
            <div class="w-full flex gap-1 flex-wrap justify-between">
                <div class="flex gap-1 flex-wrap">
                    @foreach ($bulan as $item)
                        <a href="{{ config('app.url') }}/belanja-51/wilayah/uang-lembur/{{ $thn }}/{{ $item }}"
                                ...
                                <x-table.body.column class="text-center">
                                    @if ($item->dokumenUangMakan($thn, $bln)->count() > 0)
                                        <a class="btn btn-xs btn-primary btn-outline"
                                            href="{{ config('app.url') }}/belanja-51/wilayah/uang-lembur/{{ $item->kdsatker }}/{{ $thn }}/{{ $bln }}/detail">file</a>
                                    @endif
                                </x-table.body.column>
                                <x-table.body.column class="text-center">

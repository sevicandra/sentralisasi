    ...
    <div class="h-full grid grid-rows-[auto_1fr_auto] grid-cols-1 gap-2">
        <div class="flex flex-col gap-2 py-2 px-4">
            <div class="w-full flex gap-1 flex-wrap justify-between">
                <a href="/belanja-51/wilayah/uang-makan/{{ $thn }}/{{ $bln }}"
                            ...
                            <tr class="*:border-x">
                                ...
                                <x-table.body.column class="text-center">
                                    <div class="flex gap-1">
                                        <form action="/belanja-51/wilayah/uang-makan/{{ $item->id }}/dokumen"
                                            method="post" target="_blank">
                                            @csrf
                                            @method('patch')
                                            <button class="btn btn-xs btn-primary">pdf</i></button>
                                        </form>
                                        <form action="/belanja-51/wilayah/uang-makan/{{ $item->id }}/dokumen-excel"
                                            method="post" target="_blank">
                                            @csrf
                                            @method('patch')

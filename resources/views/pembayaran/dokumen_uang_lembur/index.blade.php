...
@section('main-content')
    <div class="h-full grid grid-rows-[auto_1fr_auto] grid-cols-1 gap-2">
        <div class="flex flex-col gap-2 py-2 px-4">
            <div class="w-full flex gap-1 flex-wrap justify-between">
                <div class="flex gap-1 flex-wrap">
                    @foreach ($tahun as $item)
                        <a href="{{ config('app.url') }}/belanja-51/dokumen-uang-lembur/{{ $item->tahun }}"
                            class="btn btn-xs btn-outline btn-primary @if ($item->tahun === $thn) btn-active @endif">{{ $item->tahun }}
            ...
            <div class="w-full flex gap-1 flex-wrap justify-between">
                <div class="flex gap-1 flex-wrap">
                    @foreach ($bulan as $item)
                        <a href="{{ config('app.url') }}/belanja-51/dokumen-uang-lembur/{{ $thn }}/{{ $item->bulan }}"
                            class="btn btn-xs btn-outline btn-primary @if ($item->bulan === $bln) btn-active @endif">{{ $item->bulan }}
                            @if ($item->pending > 0)
                                <div class="badge badge-warning badge-sm">
                                    {{ $item->pending }}
                                </div>
            ...
            <div class="w-full flex gap-1 flex-wrap justify-between">
                @if ($thn && $bln)
                    <a href="{{ config('app.url') }}/belanja-51/dokumen-uang-lembur/rekap?thn={{ $thn }}&bln={{ $bln }}"
                        class="btn btn-xs btn-primary">Download Rekap</a>
                @endif
            </div>
        </div>
                    ...
                    <x-table.body>
                        @foreach ($data as $item)
                            <tr class="*:border">
                                ...
                                <x-table.body.column
                                    class="text-center">{{ $item->dokumenUangLembur($thn, $bln)->count() }}</x-table.body.column>
                                <x-table.body.column
                                    class="text-center">{{ $item->dokumenUangLembur($thn, $bln)->sum('jmlpegawai') }}</x-table.body.column>
                                <x-table.body.column class="text-center">
                                    @if ($item->dokumenUangLembur($thn, $bln)->count() > 0)
                                        <a class="btn btn-xs btn-primary btn-outline"
                                            href="{{ config('app.url') }}/belanja-51/dokumen-uang-lembur/{{ $item->kdsatker }}/{{ $thn }}/{{ $bln }}/detail">file</a>
                                    @endif
                                </x-table.body.column>
                                <x-table.body.column class="text-center">

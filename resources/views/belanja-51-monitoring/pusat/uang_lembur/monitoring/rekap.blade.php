    ...
    <div class="h-full grid grid-rows-[auto_1fr_auto] grid-cols-1 gap-2">
        <div class="flex flex-col gap-2 py-2 px-4">
            <div class="w-full flex gap-1 flex-wrap justify-between">
                <a href="/belanja-51-monitoring/pusat/uang-lembur/monitoring/{{ $thn }}/{{ $bln }}" class="btn btn-xs btn-primary">kembali</a>
            </div>
        </div>
                    ...
                    <x-table.body>
                        @foreach ($data as $item)
                            <tr class="*:border">
                                ...
                                <x-table.body.column>{{ $item->unit->nama }}</x-table.body.column>
                                <x-table.body.column
                                ...
                                <x-table.body.column class="text-center">
                                    <a href="/belanja-51-monitoring/pusat/uang-lembur/monitoring/detail/{{ $item->id }}" class="btn btn-xs btn-primary">Detail</a>
                                </x-table.body.column>
                            </tr>
                        @endforeach
                    </x-table.body>

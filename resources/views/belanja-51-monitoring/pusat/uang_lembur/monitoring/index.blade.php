                ...
                <div class="flex flex-col gap-1">
                    <div class="flex gap-1 flex-wrap">
                        @foreach ($tahun as $item)
                            <a href="{{ config('app.url') }}/belanja-51-monitoring/pusat/uang-lembur/monitoring/{{ $item }}"
                                class="btn btn-xs btn-outline btn-primary @if ((!$thn && $item == date('Y')) || $item == $thn) btn-active @endif">{{ $item }}</a>
                        @endforeach
                    </div>
                    <div class="flex gap-1 flex-wrap">
                        @foreach ($bulan as $item)
                            <a href="{{ config('app.url') }}/belanja-51-monitoring/pusat/uang-lembur/monitoring/{{ $thn }}/{{ $item }}"
                        ...
                        @foreach ($data as $item)
                            <tr class="*:border">
                                ...
                                <x-table.body.column class="text-center">
                                    {{ number_format($item->permohonanUangLemburPusat->count(), 0, ',', '.') }}
                                </x-table.body.column>
                                <x-table.body.column class="text-center">
                                    <a href="/belanja-51-monitoring/pusat/uang-lembur/monitoring/{{ $thn }}/{{ $bln }}/{{ $item->kdunit }}" class="btn btn-xs btn-primary">detail</a>
                                </x-table.body.column>
                            </tr>
                        @endforeach
                    </x-table.body>

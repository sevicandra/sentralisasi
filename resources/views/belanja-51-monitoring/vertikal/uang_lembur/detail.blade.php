    ...
    <div class="h-full grid grid-rows-[auto_1fr_auto] grid-cols-1 gap-2">
        <div class="flex flex-col gap-2 py-2 px-4">
            <div class="w-full flex gap-1 flex-wrap justify-between">
                <div>
                    <a href="/belanja-51-monitoring/vertikal/uang-lembur" class="btn btn-xs btn-primary">kembali</a>
                </div>
                <div class="flex gap-1 items-center">
                    ...
                    <form action="/{{ Request::path() }}/approve" method="post"
                        onsubmit="return confirm('Apakah Anda yakin akan menyetujui usulan ini?');">
                        @method('PATCH')
                        @csrf
        ...
        <dialog id="penolakan_modal" class="modal">
            <div class="modal-box">
                <form action="/{{ Request::path() }}/tolak" method="post"
                    onsubmit="return confirm('Apakah Anda yakin akan menolak usulan ini?');">
                    @csrf
                    @method('PATCH')
                    <div class="flex flex-col gap-2 w-full max-w-2xl">
                        <div class="flex flex-col">
                            <div class="flex flex-col">
                                <x-input name="catatan" value="{{ old('catatan') }}" label="Catatan:" size="w-full"
                                    :required="true" />
                            </div>
                        </div>

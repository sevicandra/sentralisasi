                ...
                <form action="/admin/bulan/{{ $data->id }}/update" method="post" autocomplete="off">
                    ...
                    <div class="flex flex-col gap-2 w-full max-w-2xl">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-2 gap-2">
                            <x-input type="text" name="bulan" size="w-full" value="{{ old('bulan', $data->bulan) }}"
                                label="Bulan:" />
                            <x-input type="text" name="nmbulan" size="w-full"
                                value="{{ old('nmbulan', $data->nmbulan) }}" label="Nama Bulan:" />
                        </div>

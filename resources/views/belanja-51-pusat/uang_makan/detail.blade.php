    ...
    <div class="h-full grid grid-rows-[auto_1fr_auto] grid-cols-1 gap-2">
        <div class="flex flex-col gap-2 py-2 px-4">
            <div class="w-full flex gap-1 flex-wrap justify-between">
                <a href="/belanja-51-pusat/uang-makan/permohonan" class="btn btn-xs btn-primary">kembali</a>
            </div>
        </div>
        <div class="grid grid-rows-[auto_1fr] grid-cols-1 overflow-hidden px-4 pb-2">
            ...
            <div class="h-full w-full carousel scroll-auto">
                <div class="carousel-item w-full grid grid-rows-[auto_1fr] gap-2" id="Register">
                    <div class="w-full flex justify-between gap-1 flex-wrap">
                        <div role="tablist" class="tabs tabs-bordered">
                            <a role="tab" class="tab tab-active" href="#Register">Register</a>
                            <a role="tab" class="tab" href="#Lampiran">Lampiran</a>
                        </div>
                        <div>
                            <form action="/belanja-51-pusat/uang-makan/permohonan/{{ $permohonan->id }}/regenerate" method="POST">
                                @csrf
                                @method('PATCH')
                                ...
                                <x-table.body>
                                    @foreach ($permohonan->lampiran as $item)
                                        <tr class="*:border">
                                            ...
                                            <x-table.body.column class="text-center">
                                                <form action="/belanja-51-pusat/uang-makan/permohonan/{{ $permohonan->id }}/regenerate/{{ $item->id }}" method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="btn btn-xs btn-primary">Regenerate</button>
                                                </form>
                                            </x-table.body.column>
                                        </tr>
                                    @endforeach
